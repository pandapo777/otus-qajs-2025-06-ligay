...
import { createUserBooks, replaceUserBook, gettingInformationUserBook, deleteUserBook} from "../framework/services/services.js"
...
describe('Проверка создания книги в коллекции пользователя', () =>{
    const createUserBooksCases = [
        ...
        { userId : USER_EXIST_ID, isbn : ISBN, expectedStatus: 400, expectedMessage : "ISBN already present in the User's Collection!"},
        { userId : 123, isbn : ISBN, expectedStatus: 401, expectedMessage : 'User not authorized!'},
        { userId : USER_EXIST_ID, isbn : 23135, expectedStatus: 400, expectedMessage : 'ISBN supplied is not available in Books Collection!'},
        
    ]
    test.each(createUserBooksCases)('Проверка создания книги в коллекции пользователя', async ({userId, isbn, expectedStatus, expectedMessage}) => {
    ...
    describe('Book Service', () => {
        ...
        test('3. Замена книги в коллекции пользователя', async () => {
            ...
            expect(responseBody.data.userId).toBe("d364a91a-b6b8-4477-8cf5-317c601c4436")
